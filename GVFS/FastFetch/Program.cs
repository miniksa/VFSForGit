...
namespace FastFetch
{
    public class Program
    {
        public static int Main(string[] args)
        {
            ...
            var commitOpt = new Option<string>(new[] { "-c", "--commit" }, "Commit to fetch");
            var branchOpt = new Option<string>(new[] { "-b", "--branch" }, "Branch to fetch");
            var cacheServerUrlOpt = new Option<string>("--cache-server-url", () => "", "Defines the url of the cache server");
            var chunkSizeOpt = new Option<int>("--chunk-size", () => 4000, "Sets the number of objects to be downloaded in a single pack");
            var checkoutOpt = new Option<bool>("--checkout", () => false, "Checkout the target commit into the working directory after fetching");
            var forceCheckoutOpt = new Option<bool>("--force-checkout", () => false, "Force FastFetch to checkout content as if the current repo had just been initialized.");
            var searchThreadCountOpt = new Option<int>("--search-thread-count", () => 0, "Sets the number of threads to use for finding missing blobs. (0 for number of logical cores)");
            var downloadThreadCountOpt = new Option<int>("--download-thread-count", () => 0, "Sets the number of threads to use for downloading. (0 for number of logical cores)");
            var indexThreadCountOpt = new Option<int>("--index-thread-count", () => 0, "Sets the number of threads to use for indexing. (0 for number of logical cores)");
            var checkoutThreadCountOpt = new Option<int>("--checkout-thread-count", () => 0, "Sets the number of threads to use for checkout. (0 for number of logical cores)");
            var maxRetriesOpt = new Option<int>(new[] { "-r", "--max-retries" }, () => 10, "Sets the maximum number of attempts for downloading a pack");
            var gitPathOpt = new Option<string>("--git-path", () => "", "Sets the path and filename for git.exe if it isn't expected to be on %PATH%.");
            ...
            var allowIndexMetadataOpt = new Option<bool>("--allow-index-metadata-update-from-working-tree", () => false,
                "When specified, index metadata (file times and sizes) is updated from disk if not already in the index.");
            var verboseOpt = new Option<bool>("--verbose", () => false, "Show all outputs on the console in addition to writing them to a log file");
            var parentActivityIdOpt = new Option<string>("--parent-activity-id", () => "", "The GUID of the caller - used for telemetry purposes.");
