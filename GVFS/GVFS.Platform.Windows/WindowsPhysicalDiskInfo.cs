    ...
    public class WindowsPhysicalDiskInfo
    {
        ...
        public static Dictionary<string, string> GetPhysicalDiskInfo(string path, bool sizeStatsOnly)
        {
            ...
            try
            {
                ...
                // Physical disk properties via DeviceIoControl
        ...
        {
            // Open the volume (\\.\C:) — no access rights needed for these IOCTLs
            ...
            // Resolve physical disk number from the volume
            ...
            // Open the physical drive for property queries
            ...
            // SSD vs HDD — IOCTL_STORAGE_QUERY_PROPERTY with StorageDeviceSeekPenaltyProperty
            result["PhysicalMediaType"] = QueryMediaType(diskHandle);

            // Bus type — IOCTL_STORAGE_QUERY_PROPERTY with StorageAdapterProperty
        ...
        {
            // VOLUME_DISK_EXTENTS: DWORD NumberOfDiskExtents (offset 0),
            // then DISK_EXTENT[] starting at offset 8 (4-byte padding for 8-byte alignment).
            // We only need the first extent's DiskNumber (DWORD at offset 8).
            ...
            try
            {
                        ...
                        IntPtr.Zero))
                {
                    ...
                    if (count > 0)
                    {
                        // First DISK_EXTENT.DiskNumber is at offset 8
        ...
        private static string QueryMediaType(SafeFileHandle diskHandle)
        {
            ...
            // DEVICE_SEEK_PENALTY_DESCRIPTOR layout:
            //   DWORD Version     (offset 0)
            //   DWORD Size        (offset 4)
            //   BOOLEAN IncursSeekPenalty (offset 8, 1 byte)
        ...
        private static string QueryBusType(SafeFileHandle diskHandle)
        {
            ...
            // STORAGE_ADAPTER_DESCRIPTOR layout (relevant field):
            //   ... 5 DWORDs (20 bytes) + 4 BOOLEANs (4 bytes) = offset 24
            //   BYTE BusType (offset 24)
